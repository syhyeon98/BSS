	...
	<div class="row">	
		...
		<div class="col-md-12">
			<table class="table table-hover table-sm table-bordered ">
				...
				<tbody>
					<c:forEach items="${paging.dataList }" var="mortagage" >
					<c:set  var="i" value="${i+1 }"/>
						<tr>
							<td class="align-middle">
							  <div class="form-check form-check-inline d-flex justify-content-center">
								  <input class="form-check-input prdtCheckBox" type="checkbox" name="comparePrdt" value=""
								  	data-finprdtcd="${mortagage.finPrdtCd}" 
								  	data-finprdtnm="${mortagage.finPrdtNm}"
									data-fincono="${mortagage.finCoNo }"
									data-mrtgtype="${mortagage.mrtgType }" 
									data-rpaytype="${mortagage.rpayType}"
									data-lendratetype="${mortagage.lendRateType}"
									data-loanamount="${mortagage.loanAmount }"
								  >
							  </div>
							</td>
							<td class="align-middle text-center">${mortagage.korCoNm}</td>						
							<td class="align-middle text-center">${mortagage.finPrdtNm}</td>						
							<td class="align-middle text-center">${mortagage.mrtgTypeNm}</td>						
							<td class="align-middle text-center">${mortagage.lendRateTypeNm}</td>						
							<td class="align-middle text-center">${mortagage.rpayTypeNm}</td>						
							<td class="align-middle text-center">${mortagage.lendRateMin}%</td>						
							<td class="align-middle text-center">${mortagage.lendRateMax}%</td>						
							<td class="align-middle text-center">${mortagage.lendRateAvg}%</td>						
							<td class="align-middle text-center">${mortagage.dispMonthlyPayment}</td>						
							<td class="align-middle text-center">
								<div> <a href="${mortagage.hompUrl }" target="_blank"> ${mortagage.korCoNm}</a></div>
								<div>(홈페이지)</div>
								<div>${mortagage.calTel }</div>
								<div>(대표번호)</div>
							</td>						
							<td class="align-middle text-center">
						...
						<tr class="addTable">
							...
							<div id="collapse${i }" class="collapse" aria-labelledby="heading${i }" data-parent="#accordion">
									<table class="row detailTable">
										...
										<tbody>
										<tr class="row">
												<td class="col-md-4  align-middle ">${mortagage.dclsChrgMan}</td>
												<td class="col-md-4  align-middle ">${mortagage.joinWay}</td>
												<td class="col-md-4  align-middle ">${mortagage.loanInciExpn}</td>
											</tr>
										</tbody>	
										<thead>
											<tr class="row">
										...
										<tbody>
										<tr class="row">
												<td class="col-md-4 align-middle ">${mortagage.erlyRpayFee}</td>
												<td class="col-md-4 align-middle ">${mortagage.dlyRate}</td>
												<td class="col-md-4 align-middle ">${mortagage.loanLmt}</td>
											</tr>
										</tbody>	
									</table>
								</div>
...
<script>
	...
	$(staticBackdrop).on('show.bs.modal',function(event){
		...
		selectedPrdts.forEach((prdt)=>{
			...
			mrtgTypeList.push(prdt.dataset.mrtgtype);
		...
		data = {
				...
				mrtgTypeList:mrtgTypeList,
				rpayTypeList:rpayTypeList,
				lendRateTypeList:lendRateTypeList
			};
		
			fetch(`${pageContext.request.contextPath}/mortagageLoan/compareMortagage.do`, {
			    method: "POST",
			    headers: {
