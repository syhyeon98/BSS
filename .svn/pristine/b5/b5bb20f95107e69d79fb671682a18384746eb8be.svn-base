				...
				<div class="row clearfix">
					<div class="col-md-12 col-lg-12 col-xl-12">
						<ul id="listBody" class="mail_list list-group list-unstyled">
							<c:set var="impoList" value="${paging.dataList }" />
							<c:if test="${empty impoList}">

							</c:if>
							<c:if test="${not empty impoList}">
								<c:forEach items="${impoList}" var="mail">
											...
											<div class="pull-left">
												<div class="controls">
													...
													<a href="javascript:void(0);" class="favourite text-muted hidden-sm-down" data-toggle="active">
														<i class="zmdi zmdi-star blue-star"></i>
													</a>
												</div>
												...
												<div class="media-heading">
													<c:choose>
														<c:when test="${empMail eq mail.receiveMailSender }">
															<a href="#" class="m-r-10">받는 사람 :
																${mail.receiveMailReceiver}</a>
														</c:when>
														<c:otherwise>
															<a href="#" class="m-r-10">보낸 사람 :
																${mail.receiveMailSender}</a>
														</c:otherwise>
													</c:choose>
        ...
        $(document).ready(function() {
            $("#listBody").on("click", "li.list-group-item", function(event) {
                if (!$(event.target).is('input:checkbox')) {
                    ...
                    location.href = "impoView.do?mailNo=" + mailNo;
                }
            });
    	    ...
    	    $("#listBody").on("click", ".zmdi", function(event) {
    	        ...
    	        $.ajax({
    	            ...
    	            success: function(response) {
    	                if (response == "success") {
    	                    console.log("별 상태 업데이트 성공");
    	                    $li.hide();
    	                } else {
    	                    console.error("별 상태 업데이트 실패");
    	                }
    	            },
