		...
		<div id="test"></div>
			<div class="row justify-content-end mar">
				<div class="col-auto">
					...
					<a type="button" id="helpBtn" data-bs-toggle="modal" data-bs-target="#staticBackdrop3" href="#staticBackdrop3" class="btn btn-success">참조</a>
			...
			<div class="row" id="depCondition">
						...
						<div class="col-md-11">
							<div class="form-check form-check-inline">
							  <input class="form-check-input" type="checkbox" checked id="department_chkAll" value="">
							  <label class="form-check-label" for="department_chkAll">전체</label>
							 </div>
							  <c:forEach items="${departmentList }" var="department" >
								  <div class="form-check form-check-inline">
									  <input class="form-check-input" checked type="checkbox" id="department_${department.depCd}" name="depCdList" value="${department.depCd}">
		...
		if(checkedSignpath==null || checkedSignpath==""){ // 결재선을 선택 안했을 시
			...
			let setting = {
					...
					success : function(resp) {
						...
						table+=`</tr>`
// 						참조테이블
						table +=`<tr class="textCenter"><td class="table-secondary ">참조</td><td colspan="\${appCount}"><div id="SD_03" class="d-flex">`
	...
	$("#selectHelpEmp").on("click",function(){
		...
		for(let i = 0 ; i< selectEmps.length;i++){
			...
			formInput += `<div><input type="text" class="helpInp" value="\${selectEmpCd}"  name="signPathDetailList[\${i}].empCd">
              				<input type="text" class="helpInp" value="\${selectedCode}" name="signPathDetailList[\${i}].code"></div>`
		}     
// 		$("#SD_03").append(selectEmp.innerText);
		$("#SD_03").append(html);
		...
		/* alert 시작  */
			  ...
			  hideClass: {
				...
				if(result.isConfirmed){
					...
					var msgData ={
						type: "DOC_REQ",
						signpathNo: $("input[name='signpathNo']").val(),
						msg: '결재요청'
					};
					
					console.log("htSocket", socket);
	...
	$("#tempButton").on("click",function(){
		...
		$(reqForm).append(html)
		$(reqForm).attr("action","${pageContext.request.contextPath}/ed/document/documentTempForm.do").submit();		
	})
