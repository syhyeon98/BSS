...
<script>
	...
	function callList(data) {
		$.ajax({
					url : `${pageContext.request.contextPath}/finMarket/internalIntrRatesList`,
					type : 'POST',
					dataType : 'json',
	...
	function firstCallList() {
		$.ajax({
					url : `${pageContext.request.contextPath}/finMarket/firstInternalIntrRates`,
					type : 'GET',
					dataType : 'json',
					success : function(result) {
						makeBarChart(result);
